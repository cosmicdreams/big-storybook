...
import '../../css/baseline/performance.css';
...
const getNavigationMetrics = () => {
  ...
  if (!nav) return { ttfb: '0', dom: '0', load: '0' };
...
export const ResourceWaterfall = {
  render: (args) => {
    ...
    const maxDuration = Math.max(...resources.map(r => parseFloat(r.start) + parseFloat(r.duration)), 1);
