...
import '../../css/baseline/navigation.css';
...
const NavigationTemplate = (args) => {
  ...
  `;
    ...
    const routes = {
        ...
        features: { title: 'Async Navigation', content: 'Native support for intercepting navigations.' },
    ...
    const renderRoute = (path) => {
        ...
        container.querySelectorAll('.nav-link').forEach(l => l.classList.toggle('active', l.dataset.route === path));
        if (window.navigation) renderHistory();
    };

    const renderHistory = () => {
        const list = container.querySelector('#entries-list');
        if (list) {
            list.innerHTML = navigation.entries().map(entry => `
              <div class="history-entry ${entry === navigation.currentEntry ? 'active' : ''}">
    ...
    if (window.navigation) {
        ...
        navigation.addEventListener('navigate', (e) => {
            if (!e.canIntercept || e.downloadRequest) return;
            e.intercept({
                async handler() {
    ...
    container.addEventListener('click', (e) => {
        ...
        if (link && window.navigation) {
            e.preventDefault();
            navigation.navigate(link.href, { state: args?.state });
        }
    });
